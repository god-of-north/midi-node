...
class MousePotentiometer:
    def __init__(self, name: str, analog_pin_id: int, actions: dict, calibration: PotCalibration):
        self.name = name
    ...
    def update_value(self, delta_x: int, sensitivity: float = 1.0):
        ...
        # We need to scale it and add it to current_value, clamping within [min_value, max_value].
        change = int(delta_x * sensitivity)
        new_value = self.current_value + change
        self.current_value = max(self.calibration.min_value, min(self.calibration.max_value, new_value))
    ...
    def process_value(self, threshold: int):
        ...
        current_value = self.ema_filter.filter(self.current_value)
        ...
        if current_value <= self.calibration.min_threshold and not self.is_min:
            self._fire_event(PotEvent.ON_MIN, current_value)
            self.is_min = True
        elif current_value > self.calibration.min_threshold and self.is_min:
            self._fire_event(PotEvent.LEAVE_MIN, current_value)
            self.is_min = False

        if current_value >= self.calibration.max_threshold and not self.is_max:
            self._fire_event(PotEvent.ON_MAX, current_value)
            self.is_max = True
        elif current_value < self.calibration.max_threshold and self.is_max:
...
class MouseInputHandler(InputHandler):
    ...
    def add_potentiometer(self, name: str, analog_pin: int, actions: dict, calibration: PotCalibration):
        ...
        pot = MousePotentiometer(name, analog_pin, actions, calibration)
