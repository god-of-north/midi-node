...
class PresetActionSelectorState(MenuState):
    """
    State for selecting the Action type used as Preset Enter/Exit action.
    """
    ...
    def handle_event(self, event):
        if event.type == EventType.ENCODER_SELECT:
            ...
            existing_action: Action = preset.enter_action if self.is_enter else preset.exit_action
            if not existing_action or existing_action.__class__ != new_action_type.action_cls:
                if self.is_enter:
                    preset.enter_action = new_action_type.action_cls(context=self.context)
                else:
                    preset.exit_action = new_action_type.action_cls(context=self.context)
